...
public final class WorldPacketsDecoder extends Decoder {
	...
	public static void decodeLogicPacket(final Player player, LogicPacket packet) {
		...
		if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
			...
			if ((player.isIronMan && !p2.isIronMan) || (!player.isIronMan && p2.isIronMan)) {
				player.getPackets().sendGameMessage("Iron Man cannot trade with economy players.");
				return;
			}
			if (player.isIronMan) {
				player.getPackets().sendGameMessage("Iron Man cannot trade");
				return;
			}
			if (p2.isIronMan) {
				player.getPackets().sendGameMessage("Iron Man cannot trade");
			...
			if ((player.isIronMan && !p2.isIronMan) || (!player.isIronMan && p2.isIronMan)) {
				player.getPackets().sendGameMessage("IronMan cannot attack economy players.");
				return;
			}
			...
			case 747:
				...
				if ((interfaceId == 747 && componentId == 15)
					...
					if ((player.isIronMan && !p2.isIronMan) || (!player.isIronMan && p2.isIronMan)) {
						player.getPackets().sendGameMessage("Iron man cannot attack economy players.");
						return;
					}
					if (!player.isCanPvp() || !p2.isCanPvp()) {
						player.getPackets()
	...
	public void processPackets(final int packetId, InputStream stream,
		...
		player.afkTimer = Utils.currentTimeMillis() + (15*60*1000);
			...
			if (message.startsWith("nigger") || message.startsWith("paki")) {
			player.isRacist++;
			player.getPackets().sendGameMessage("You've just said a racist word, after saying 3 you will be muted.");
			player.getPackets().sendGameMessage("Racism count "+player.isRacist+"/3");
			return;
			}
