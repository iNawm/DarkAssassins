...
import com.rs.game.player.actions.mining.StarMining;
...
import com.rs.game.player.content.GrandExchange;
...
import com.rs.game.player.content.ShootingStars.StarStages;
	...
	private static void handleOption1(final Player player, final WorldObject object) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (id == 26439) {
					Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2887, 5345, 0));
					player.getControlerManager().startControler("GodWars");
					player.getControlerManager().startControler("GodWars");
			}
				if (id == 3634) {
					...
					if (player.starterstage == 3) {
		                ...
		                for (Player players : World.getPlayers()) {
		                    ...
		                    if (player.isPker) {
								...
								players.getPackets().sendGameMessage("<img=6><col=FF0000>All welcome "+ player.getDisplayName() +", they are playing as Iron Man on" +gameMode+ " XP rate! </col></img>");
			               		 player.Rewardpoints++;
			               		 player.Forumpoints++;
			               		 player.lividpoints++;
							...
							String moof2 = "adventure on Zamron!"; /*
							try {
						...
						player.sendMessage("You must talk with the Zamron guide Priest first.");
				}
 ...
 if (id == 27896 || id == 32046 || id == 31747 || id == 30838
   ...
   HalloweenObject.HandleObject(player, object);
			...
			if (object.getId() == 26341) { //godwars dungeon.
				player.useStairs(828, new WorldTile(2881, 5310, 0), 0, 0);
				player.getControlerManager().startControler("GodWars");
			return;
		}
			
			if (SoulLobby.handleObjects(player, id))
				return;
			...
			if (id == 62414) {
				if (player.getUsername().equalsIgnoreCase("Leighton") || player.getUsername().equalsIgnoreCase("") || player.getUsername().equalsIgnoreCase("") ) {
				...
				if (id == 38660){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.SECOND));
					return;
				}
				if (id == 38661){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.THIRD));
					return;
				}
				if (id == 38662){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.FORTH));
					return;
				}
				if (id == 38663){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.FIVE));
					return;
				}
				if (id == 38664){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.SIX));
					return;
				}
				if (id ==38665){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.SEVEN));
					return;
				}
				if (id == 38666){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.EIGHT));
					return;
				}
				if (id == 38667){
					player.getActionManager().setAction(
							new StarMining(object, StarStages.NINE));
					return;
				}
				if (id == 38668){
...
Magic.pushLeverTeleport(player,
	...
	player.useStairs(827, new WorldTile(4717, 5465, 0), 1, 2);
...
player.getPackets().sendGameMessage("You need at least 71 agility to climb this.");
					...
					player.getPackets().sendGameMessage(
							"You examine portal and it absorbs you...");
				} else if (id == 9356) 
				...
				//start dagganoth lair
					...
					Magic.pushLeverTeleport(player, new WorldTile(1812, 4406, 2));
				
				
				/*else if (id == 10196)
					player.setNextWorldTile(new WorldTile(1806, 4406, 3));*/
					...
					Magic.pushLeverTeleport(player, new WorldTile(1825, 4404, 3));
				
				
				else if (id == 10199)
					//player.setNextWorldTile(new WorldTile(1834, 4388, 2));
					Magic.pushLeverTeleport(player, new WorldTile(1834, 4388, 2));
