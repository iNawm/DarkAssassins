	...
	public void makePoisoned(int startDamage) {
		...
		if (entity instanceof Player) {
			...
			if (player.getEquipment().getShieldId() == 18340) {
				player.getPackets().sendGameMessage("Your anti-poison totem saves you from the poison.");
				return;
			}
			if (player.healthBath) {
				player.getPackets().sendGameMessage("You are currently immune to poison.");
				return;
			}
