...
public class HungerGames {
	...
	public static final WorldTile OUTSIDE = new WorldTile(3978, 4180, 1);
	...
	private static WorldTile mapStart(int i) {
		inGame = true;
		return new WorldTile(3801 + i, 3530, 0);
	}
	
	private static void placeObjects(WorldTile mapStart) {
		if (mapStart != null) {
			WorldObject obj = new WorldObject(4411, 22, -1, mapStart.getX(),
					mapStart.getY(), mapStart.getPlane());
			World.spawnObject(obj, true);
			objects.add(obj);
		} else {
			for(int i = 0; i < 20; i++) {
				WorldTile changingTile = null;
				if(i == 0)
				      changingTile = new WorldTile(3784, 3550, 0);
				else if(i == 1)
				      changingTile = new WorldTile(3790, 3553, 0);
				else if(i == 2)
					  changingTile = new WorldTile(3798, 3554, 0);
				else if(i == 3)
					  changingTile = new WorldTile(3803, 3550, 0);
				else if(i == 4)
					  changingTile = new WorldTile(3810, 3551, 0);
				else if(i == 5)
					  changingTile = new WorldTile(3810, 3557, 0);
				else if(i == 6)
					  changingTile = new WorldTile(3813, 3562, 0);
				else if(i == 7)
					  changingTile = new WorldTile(3819, 3565, 0);
				else if(i == 8)
					  changingTile = new WorldTile(3813, 3571, 0);
				else if(i == 9)
					  changingTile = new WorldTile(3813, 3532, 0);
				else if(i == 10)
					  changingTile = new WorldTile(3817, 3529, 0);
				else if(i == 11)
					  changingTile = new WorldTile(3809, 3544, 0);
				else if(i == 12)
					  changingTile = new WorldTile(3816, 3547, 0);
				else if(i == 13)
					  changingTile = new WorldTile(3818, 3558, 0);
				else if(i == 14)
					  changingTile = new WorldTile(3806, 3560, 0);
				else if(i == 15)
					  changingTile = new WorldTile(3797, 3558, 0);
				else if(i == 16)
					  changingTile = new WorldTile(3795, 3549, 0);
				else if(i == 17)
					  changingTile = new WorldTile(3787, 3557, 0);
				else if(i == 18)
					  changingTile = new WorldTile(3824, 3550, 0);
				else if(i == 19)
					  changingTile = new WorldTile(3817, 3527, 0);
				else if(i == 20)
					  changingTile = new WorldTile(3808, 3526, 0);
				WorldTile randomTile = new WorldTile(changingTile, 2);
				if (!World.canMoveNPC(randomTile.getPlane(), randomTile.getX(),
						randomTile.getY(), 1))
					return;
	...
	public static void placePlayer(final Player player) {
		...
		for (int i = 0; players.size() > i; i++)
			if (players.get(i) == player) {
				...
				player.setNextWorldTile(mapStart(i));
			}
