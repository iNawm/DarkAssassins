 ...
 * @since Nov 30, 2013
 */
public class FistOfGuthixGame extends Controler {
	...
	public void destroy(Player player, final boolean logout, int mode) {
		if (logout)
			...
			player.setNextWorldTile(FistOfGuthixConfiguration.HALL_LOCATION);
		}
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
	...
	public void addPlayerToGame(Player p) {
		p.setNextWorldTile(new WorldTile(getBaseX() + randomTile, getBaseY() + randomTile, 0));
	}
}
