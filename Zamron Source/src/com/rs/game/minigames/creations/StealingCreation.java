...
public class StealingCreation {
	...
	public static boolean proccessKilnItems(Player player, int componentId, int index, int itemId, int amount) {
		...
		if (player.getInventory().containsItem(clayId, 1)) {
			if (player.getInventory().addItemMoneyPouch(new Item(StealingCreation.CLASS_ITEMS_BASE[componentId - 37] + ((componentId == 57 || componentId == 58 || componentId == 61) ? 0 : componentId == 56 ? index : componentId >= 64 ? (-index * 2) : (index * 2)), (componentId >= 56 && componentId <= 58 ? 15 * (index + 1) : componentId == 61 ? index + 1 : 1) * amount))) {
	...
	private static boolean canEnter(Player player, boolean inRedTeam) {
		getTotalLevel(TOTAL_SKILL_IDS, inRedTeam);
		getTotalLevel(TOTAL_COMBAT_IDS, inRedTeam);
		getTotalLevel(TOTAL_SKILL_IDS, !inRedTeam);
		getTotalLevel(TOTAL_COMBAT_IDS, !inRedTeam);
		/*if ((skillTotal + combatTotal) > (otherSkillTotal + otherCombatTotal)) {
			...
			return false;
		} else */if (player.getEquipment().wearingArmour() || player.getInventory().getItems().getSize() == 0 || player.getFamiliar() != null) {
