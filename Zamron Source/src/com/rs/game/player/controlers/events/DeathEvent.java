...
/*	    new WorldTile(3222, 3219, 0),
    ...
    public static final WorldTile[] RESPAWN_LOCATION = { 
    	Settings.RESPAWN_DEATH_LOCATION
    	//	new WorldTile(3222, 3219, 0), 
    //	new WorldTile(2971, 3343, 0), 
    //	new WorldTile(2758, 3486, 0), 
    //	new WorldTile(1891, 3177, 0), 
    //	new WorldTile(2889, 3528, 0) 
    };
    ...
    public boolean logout() {
		player.setLocation(Settings.RESPAWN_DEATH_LOCATION);
		destroyRoom();
		return false;
    }
    ...
    public boolean processObjectClick1(WorldObject object) {
		if (object.getId() == 45803) {
		    if (getArguments() == null || getArguments().length < 2 || player.getRights() == 2)
		    	Magic.sendObjectTeleportSpell(player, true, Settings.RESPAWN_DEATH_LOCATION);
		    else
		    	getReadyToRespawn();
		    return false;
		}
    ...
    public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		if (interfaceId == 18) {
			...
			if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
				...
				if (slotId > RESPAWN_LOCATION.length)
				    return false;
    ...
    public void getReadyToRespawn() {
    	//slots = GraveStoneHandler.getItemSlotsKeptOnDeath(player, false, hadSkull(), player.getPrayer().isProtectingItem());
    	...
    	player.getPackets().sendUnlockIComponentOptionSlots(18, 45, 0, RESPAWN_LOCATION.length, 0);
    	player.setCloseInterfacesEvent(new Runnable() {
    	    @Override
    	    public void run() {
	    		WorldTile respawnTile = currentHub >= 256 ? RESPAWN_LOCATION[currentHub - 256] : HUBS[currentHub];
	    		synchronized (slots) {
    ...
    public WorldTile getDeathTile() {
		if (getArguments() == null || getArguments().length < 2)
		    return Settings.RESPAWN_DEATH_LOCATION;
