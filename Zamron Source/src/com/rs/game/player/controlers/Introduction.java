 ...
 */
public class Introduction extends Controler implements Serializable {
    ...
    public void start() {
        ...
        if (stage == 0) {
            ...
            player.setNextWorldTile(new WorldTile(0,0,0));
    ...
    private void generateRegion() {
        ...
        player.setNextWorldTile(getWorldTile(27, 34, 1));
        nurse = new NPC(15017, new WorldTile(player.getX(), player.getY() + 1, player.getPlane()), -1, false);
        nurse.spawn();
    ...
    public void doStageAction() {
        switch (stage) {
            case 3:
                ...
                break;
            case 4:
                player.setNextWorldTile(getWorldTile(36, 25, 0));
                guard.setNextWorldTile(getWorldTile(37, 25, 0));
                player.faceEntity(guard);
                guard.faceEntity(player);
                break;
            case 5:
                FadingScreen.fade(player, new Runnable() {
                    @Override
                    public void run() {
                        player.setNextWorldTile(new WorldTile(3212, 3425, 0));
                        player.unlock();
