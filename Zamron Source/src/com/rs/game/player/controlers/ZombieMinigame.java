	...
	public void loadCave(final boolean login) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				player.setNextWorldTile(!login ? getWorldTile(11, 10) : getWorldTile(11, 10) );
				//1delay because player cant walk while teleing :p, + possible issues avoid
				WorldTasksManager.schedule(new WorldTask()  {
					@Override
					public void run() {
						if(!login) {
							WorldTile walkTo = getWorldTile(11, 10);
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					player.reset();
					player.zombie += 1;
					player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "You have defeated every wave! You have recieved a zombie point! You now have "+player.zombie+" zombie points.");
