...
public class PestInvasion extends Controler {
	...
	public void setWaveEvent() {
		...
		if(getCurrentWave() == 48) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", VOID_KNIGHT, "Watch out, this is the deadliest mage on Zamron!");
		player.getInventory().addItem(23351, 5);
		}	
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					player.addPestInvasionPoints(player.getPestinvPoints() + points * bonus *2);
					...
					player.getDialogueManager().startDialogue("SimpleNPCMessage", VOID_KNIGHT, "Well done in the cave, here, take" + points + " PestInvasion points as reward.");
