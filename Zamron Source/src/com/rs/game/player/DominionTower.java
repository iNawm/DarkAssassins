...
public final class DominionTower implements Serializable {
	...
	public void win(int mode) {
		...
		killedBossesCount++;
		player.domCount++;
		player.dominionFactor += factor;
		totalScore += factor;
		if (player.dominionFactor > MAX_FACTOR) {
			player.dominionFactor = MAX_FACTOR;
			player.getPackets()
					.sendGameMessage(
							"You've reached the maximum Dofminion Factor you can get so you should spend it!");
		}
	...
	public void openEnduranceMode() {
		...
		player.getPackets().sendIComponentText(1173, 29,
				String.valueOf(player.dominionFactor)); // current
		player.getPackets().sendIComponentText(
				1173,
				31,
				player.dominionFactor == MAX_FACTOR ? "" : String
						.valueOf(getBossesTotalLevel() * 10)); // on win
		player.getPackets().sendIComponentText(1173, 33,
				String.valueOf(player.dominionFactor)); // on death
	}
	...
	public void talkToFace() {
		...
		public void talkTochest() {
		talkToChest(false);
	}
	public void talkToChest(boolean fromDialogue) {
