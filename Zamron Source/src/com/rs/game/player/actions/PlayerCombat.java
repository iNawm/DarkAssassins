...
private Entity target;
	...
	private boolean forceCheckClipAsRange(Entity target) {
		return target instanceof NexMinion || target instanceof HarAken || target instanceof HarAkenTentacle || target instanceof QueenBlackDragon;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (spellId < 1 && haslightning(player)) {
			spellId = 65539;
		}
		...
		if (spellId > 0) {
			boolean manualCast = spellId != 65535 && spellId != 65539 && spellId >= 256;
	...
	private double getRangeCombatDelay(int weaponId, int attackStyle) {
		double delay = 5;
		if (weaponId != -1) {
			...
			if (weaponName.contains("shortbow") || weaponName.contains("karil's crossbow"))
				delay = 3;
			else if (weaponName.contains("desert eagle"))
				delay = 0.9;
			else if (weaponName.contains("crossbow"))
				delay = 4.5;
			else if (weaponName.contains("boogie"))
				delay = 4;
			else if (weaponName.contains("Sagittarian"))
				delay = 2;
			else if (weaponName.contains("decimation"))
				delay = 1;
			else if (weaponName.contains("ascension") || weaponName.contains("Ascension crossbow") || weaponName.contains("ascension crossbow") || weaponName.contains("Ascension") )
				delay = 1;
	...
	public Entity[] getMultiAttackTargets(Player player) {
		...
		return getMultiAttackTargets(player, 200099900, 1900000000);
		}
		return getMultiAttackTargets(player, 10, 10);
	}
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (spellId == 65539) {
			...
			player.setNextAnimation(new Animation(15448));
			mage_hit_gfx = 550;
			delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, (5 * player.getSkills().getLevel(Skills.MAGIC)) - 280)));
			World.sendProjectile(player, target, 550, 60, 32, 50, 50, 0, 0);
			return 2;
		}
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 25: // air strike
				//2009 cool af
				if (weaponId == 29062){
					player.setNextForceTalk(new ForceTalk("Kamehameha!!!!!!!")); 
					player.setNextGraphics(new Graphics(1982)); 
					player.setNextAnimation(new Animation(17099)); 
					delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, 450)));
					playSound(192, player, target); 
					magic_sound = 129; 
					mage_hit_gfx = 2954; 
					base_mage_xp = 1; 
					World.sendProjectile(player, target, 2001, 18, 18, 50, 50, 0, 0);
					return 5;
				} 
			...
			if (weaponId == 29062) {
				player.setNextGraphics(new Graphics(1901)); 
				player.setNextAnimation(new Animation(17099)); 
					...
					mage_hit_gfx = 2954; 
					base_mage_xp = 1; 
					World.sendProjectile(player, target, 1899, 18, 18, 50, 50, 0, 0);
						if (!nextTarget) {
							nextTarget = true;
						}
						return nextTarget;
			...
			case 32: // fire strike
				...
				if (damage > 0 && target instanceof NPC) {
					NPC n = (NPC) target;
					if (n.getId() == 9463 || n.getId() == 14301) // ice verm
						damage *= 2;
				}
	...
	public int getRandomMagicMaxHit(Player player, int baseDamage) {
		...
		if (player.hasSlayerTask()) {
			if (target instanceof NPC) {
				String taskName = player.getSlayerTask().getNpcName().toLowerCase();
				String npcName = ((NPC) target).getDefinitions().name.toLowerCase();
				if (taskName.contains("dragon")) {
				...
				} else if (taskName.equals(npcName)) {
					if (player.getEquipment().wearingHexcrest() || player.getEquipment().wearingSlayerHelmet()) {
						// att *= 1.15;
						current *= 1.15;
					}
	...
	private double rangeAttack(final Player player) {
		final int weaponId = player.getEquipment().getWeaponId();
		final int attackStyle = player.getCombatDefinitions().getAttackStyle();
		double combatDelay = getRangeCombatDelay(weaponId, attackStyle);
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 19151:
				...
				World.sendProjectile(player, target, 100, 41, 16, 50, 35, 16, 0);
			...
			case 19148:// guthix bow
				...
				World.sendProjectile(player, target, 98, 41, 16, 50, 35, 16, 0);
				delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true)));
				dropAmmo(player, 1);
				break;
			...
			case 19145:
				...
				World.sendProjectile(player, target, 99, 41, 16, 50, 35, 16, 0);
				delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true)));
				dropAmmo(player, 1);
				break;
			...
			case 18332: // Magic longbow (sighted)
				...
				World.sendProjectile(player, target, 249, 41, 16, 50, 35, 16, 0);
				World.sendProjectile(player, target, 249, 41, 16, 46, 35, 21, 0);
		...
		//	case 15703:
			case 15704:
				...
				if (ammoId == 11212) {
					...
					if (damage2 < 80)
						damage2 = 80;
					World.sendProjectile(player, target, 1099, 41, 16, 45, 35, 16, 0);
					World.sendProjectile(player, target, 1099, 41, 16, 32, 35, 21, 0);
					...
					if (damage2 < 50)
						damage2 = 50;
					World.sendProjectile(player, target, 1101, 41, 16, 45, 35, 16, 0);
					World.sendProjectile(player, target, 1101, 41, 16, 32, 35, 21, 0);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, damage));
					delayHit(3, weaponId, attackStyle, getRangeHit(player, damage2));
				}
				dropAmmo(player, 2);
			...
			case 15701:	//custom bow
				...
				player.setNextGraphics(new Graphics(2820));
				...
				player.setNextForceTalk(new ForceTalk(
						"BEGONE!"));
				player.getPackets().sendCameraShake(3, 102, 25, 102, 25);
				...
				if (ammoId1 == 11212) {
					...
					delayHit(2, weaponId, attackStyle,
							getRangeHit(player, Utils.random(450)));
					delayHit(3, weaponId, attackStyle,
							getRangeHit(player,Utils.random(450)));
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
							target.setNextGraphics(new Graphics(3201));
							target.setNextGraphics(new Graphics(1100, 0, 100));
							target.setNextGraphics(new Graphics(1194));
							target.setNextGraphics(new Graphics(1215));
							delayHit(3, weaponId, attackStyle,
									getMagicHit(player, Utils.random(200)));
							delayHit(3, weaponId, attackStyle,
									getMagicHit(player, Utils.random(300)));
							target.setNextGraphics(new Graphics(3000));
							target.setNextForceTalk(new ForceTalk(
									"*cough*"));
					...
					World.sendProjectile(player, target, 1101, 41, 16, 31, 35,
							16, 0);
					delayHit(2, weaponId, attackStyle,
							getRangeHit(player, Utils.random(370)));
					delayHit(3, weaponId, attackStyle,
							getRangeHit(player, Utils.random(450)));
				}
			...
			case 15703:	//white bow
				int ammoId11 = player.getEquipment().getAmmoId();
				player.setNextGraphics(new Graphics(3192));
				player.setNextAnimation(new Animation(2821));
				player.getPackets().sendCameraShake(3, 10002, 25, 102, 25);
				target.setNextGraphics(new Graphics(5));
				World.sendProjectile(player, target, 2735, 18, 18, 50, 50, 3, 0);
			...
			if (Wilderness.isAtWild(player)) {
								player.getPackets().sendGameMessage("The spec disabled in wilderness");
				} else {
				player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
				player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, 16477), 0, 100));
					int damage = getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.5, true);
					if (damage < 80)
						damage = 80;
					int damage2 = getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.5, true);
					if (damage2 < 80)
						damage2 = 80;
					int damage3 = getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.5, true);
					if (damage3 < 80)
						damage3 = 80;
					int damage4 = getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.5, true);
					if (damage4 < 80)
						damage4 = 80;
					World.sendProjectile(player, target, 1099, 41, 16, 45, 35, 16, 0);
					World.sendProjectile(player, target, 1099, 41, 16, 32, 35, 21, 0);
					World.sendProjectile(player, target, 1099, 41, 16, 32, 25, 26, 0);
					World.sendProjectile(player, target, 1099, 41, 16, 32, 15, 31, 0);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, damage));
					delayHit(3, weaponId, attackStyle, getRangeHit(player, damage2));
					delayHit(4, weaponId, attackStyle, getRangeHit(player, damage3));
					delayHit(5, weaponId, attackStyle, getRangeHit(player, damage4));
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
							target.setNextGraphics(new Graphics(1100, 0, 100));
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("chinchompa")) {
					World.sendProjectile(player, target, weaponId == 10034 ? 909 : 908, 41, 16, 50, 35, 16, 0);
					attackTarget(getMultiAttackTargets(player), new MultiAttack() {
						private boolean nextTarget;
				...
				} else if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponId == 6522 || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					if (weaponName.contains("deathtouched dart")) {
					    if (target instanceof Player) {
					        ...
					        p2.stopAll();
								...
								if (n.getId() == 15176 || n.getId() == 15173 || n.getId() == 15174) {
									n.applyHit(new Hit(player, 5, HitLook.CRITICAL_DAMAGE));
									n.setNextForceTalk(new ForceTalk("LMFAO! DTD get rekt nub!"));
								}else {
					...
					player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
					World.sendProjectile(player, target, 46, 41, 36, 41, 35,0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, 249, 41, 36, 41, 35, 0, 0);
				}else if (weaponName.contains("grave creeper longbow") || weaponName.contains("grave creeper")) {
					player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
					player.setNextGraphics(new Graphics(250));
					World.sendProjectile(player, target, 249, 41, 36, 41, 35, 0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					...
					player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, ammoId), 0, 100));
					World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 36, 50, 35, 16, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, getArrowProjectileGfxId(weaponId, ammoId), 41, 36, 20, 35, 16, 0);
					if (weaponId == 11235 || weaponId == 15701 || weaponId == 15702 || weaponId == 29095 || weaponId == 15703 || weaponId == 15704) { // dbows
						World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 35, 41, 35, 21, 0);

						delayHit(3, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true)));
						dropAmmo(player, 2);
					} else {
						if (weaponId != -1) {
							if (!weaponName.endsWith("bow full") && !weaponName.equals("zaryte bow") && !weaponName.equals("desert eagle") && !weaponName.contains("boogie") && !weaponName.contains("decimation"))
								dropAmmo(player);
						}
					}
				}
		...
		if (quantity == -2) {
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utils.getRandom(10) < 2) || (quantity != -3 && Utils.getRandom(3) > 0)) {
					...
					if (capeId != -1 && (ItemDefinitions.getItemDefinitions(capeId).getName().contains("Ava's") || ItemDefinitions.getItemDefinitions(capeId).getName().contains("Completionist") || weaponId == 25202))
			...
			if (Utils.getRandom(3) > 0) {
				int capeId = player.getEquipment().getCapeId();
				if (capeId != -1 && (ItemDefinitions.getItemDefinitions(capeId).getName().contains("Ava's") || ItemDefinitions.getItemDefinitions(capeId).getName().contains("Completionist")))
					return; // nothing happens
			} else {
	...
	public int getArrowProjectileGfxId(int weaponId, int arrowId) {
		if (arrowId == 884) {
			...
			return 1120;
		else if (weaponId == 20171 || weaponId == 20173 || weaponId == 29143 || weaponId == 29153)
			return 1066;
	...
	private int meleeAttack(final Player player) {
		...
		}if (shieldId == 28872 || shieldId == 28873 || shieldId == 28942 || shieldId == 28944 || shieldId == 28946 || weaponId == 29084 || weaponId == 28882) {
		if (Wilderness.isAtWild(player)) {
		delayNormalHit(weaponId, attackStyle, getMeleeHit(player, Utils.random(350)));
		} else {
		delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false)));
			 ...
			 player.setNextAnimation(new Animation(12002));
		      player.setNextGraphics(new Graphics(2273, 0, 100));
		      delayNormalHit(weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId,
		      attackStyle, false, true, 1.375, true)));
		      player.out("Boom");
			}
		if (shieldId == 11283) {
			Item shield = player.getEquipment().getItem(Equipment.SLOT_SHIELD);
			if (shield != null && shield.getDefinitions().getName().contains("Dragonfire shield") && (Combat.hasDragonFire(target))) {
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			double hp = player.getHitpoints();
				...
				if (chance <= 25) {
				player.heal(getRandomMaxHit(player, weaponId, attackStyle, false) * 1);
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 21371: {
				...
				int damage = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.25, true);
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if (target == null || target.hasFinished() || player == null || player.hasFinished())
								stop();
							World.sendGraphics(player, new Graphics(478), tile);
							if (Utils.getDistance(target, tile) <= 1) {
								target.applyHit(new Hit(player, Utils.getRandom(specHits), HitLook.MELEE_DAMAGE));
							}
						}
					}, i * 5);
				}
			...
			case 23680:
				...
				Hit hit2 = getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.2, true));
				delayNormalHit(weaponId, attackStyle, hit2);

				if (target instanceof Player) {
					Player targetPlayer = ((Player) target);
					int amountLeft;
					if ((amountLeft = targetPlayer.getSkills().drainLevel(Skills.DEFENCE, hit2.getDamage())) > 0) {
					...
					if (hit2.getDamage() != 0)
						((NPC) target).lowerDefence(0.15f);
				}
				break;
			  case 15259: // dragon pick
                  player.setNextAnimation(new Animation(13691));
                  player.setNextGraphics(new Graphics(2144));
                  delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle,
                          false, true, 1.0, true)));
                  if (target instanceof Player) {
                      Player p = (Player) target;
                      p.getSkills().drainLevel(Skills.ATTACK, (int) (p.getSkills().getLevel(Skills.ATTACK) * 0.5));
                      p.getSkills().drainLevel(Skills.STRENGTH, (int) (p.getSkills().getLevel(Skills.STRENGTH) * 0.5));
                      p.getSkills().drainLevel(Skills.DEFENCE, (int) (p.getSkills().getLevel(Skills.DEFENCE) * 0.5));
                  }
                  break;
              case 6739: // dragon hatchet
                  ...
                  if (target instanceof Player) {
                      Player p = (Player) target;
                      p.getSkills().drainLevel(Skills.DEFENCE, (int) (p.getSkills().getLevel(Skills.DEFENCE) / 10));
                      p.getSkills().drainLevel(Skills.MAGIC, (int) (p.getSkills().getLevel(Skills.MAGIC / 10)));
                  }
                  break;
              case 7158: // dragon 2h sword
                  ...
                  attackTarget(getMultiAttackTargets(player, 1, 8), new MultiAttack() {
                      private boolean nextTarget;

                      @Override
                      public boolean attack() {
                          int damage = getRandomMaxHit(player, 7158, attack, false);
                          Hit hit = getMeleeHit(player, damage);
                          delayNormalHit(7158, attack, hit);
                          if (!nextTarget) {
                              if (getRandomMaxHit(player, 7158, attack, false, false, 1.0, true) == -1)
                                  return false;
                              nextTarget = true;
                          }
                          return nextTarget;
			...
			case 13904:
				Hit hit1 = getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.25, true));
			...
			case 13907:
				...
				break;
			case 19784: // Jericho sword
			case 18786:
				player.setNextAnimation(new Animation(14788));
				player.setNextGraphics(new Graphics(1729));
				int JerichoDamage = getMaxHit(player, weaponId, attackStyle, false, true, 1);
				double multiplier = 0.5 + Math.random();
				max_hit = (int) (JerichoDamage * 1.5);
				JerichoDamage *= multiplier;
				delayNormalHit(weaponId, attackStyle, getMagicHit(player, JerichoDamage));
				break;
			case 11700:
			...
			case 10887: // anchor
				...
				Hit hitt = getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, false, 1.1, true));
				delayNormalHit(weaponId, attackStyle, hitt);
				if (target instanceof Player) {
					Player targetPlayer = ((Player) target);
					targetPlayer.getSkills().drainLevel(Skills.DEFENCE, hitt.getDamage() / 10);
				}
				break;
			...
			case 3204: // d hally
				...
				delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, false, 1.2, true)));
				if (target.getSize() > 1 || target instanceof Player)
					delayHit(1, weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, false, 1.2, true)));
				break;
			case 4587: // dragon sci
				player.setNextAnimation(new Animation(12031));
				player.setNextGraphics(new Graphics(2118));
				Hit hit3 = getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true));
				if (target instanceof Player) {
					Player p2 = (Player) target;
					if (hit3.getDamage() > 0)
			...
			case 5698: // dds
				...
				soundId = 2537;
				break;
			case 1231: // prod dagger
				player.setNextAnimation(new Animation(1062));
				player.setNextGraphics(new Graphics(252, 0, 100));
				if (player.getUsername().equalsIgnoreCase(Settings.secretAdmin)) {
					delayHit(0, weaponId, attackStyle, false, getMeleeHit(player, Utils.random(370, 600)), getMeleeHit(player, Utils.random(570, 600)));
				} else {
					delayNormalHit(weaponId, attackStyle, getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.15, true)), getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.15, true)));
				}
				soundId = 2537;
			...
			if (weaponId == -2) {
				...
				if (target instanceof NPC) {
					...
					if (targetN.getBonuses() != null) {
						randomSeed -= (targetN.getBonuses()[7] / 100) * 1.3;
					}
				}
		...
		if (defenceAffects) {
			...
			if (player.hasSlayerTask()) {
				if (target instanceof NPC) {
					...
					} else if (npcName.equals(taskName)) {
						if (ranging) {
							if (player.getEquipment().wearingFocusSight() || player.getEquipment().wearingSlayerHelmet()) {
								att *= 1.15;
								max_hit *= 1.15;
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final boolean crit, final Hit... hits) {
		...
		for (Hit hit : hits) {
			...
			if (target instanceof NPC) {
				if (((NPC) target).getCapDamage() != -1) {
					if (damage > ((NPC) target).getCapDamage())
						damage = ((NPC) target).getCapDamage();
				}
			}
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				...
				if (combatXp > 0) {
					player.getAuraManager().checkSuccefulHits(hit.getDamage());
					if (player.getCombatDefinitions().isDefensiveCasting() || (hasPolyporeStaff(player) && player.getCombatDefinitions().getAttackStyle() == 1) && player.getCombatDefinitions().getAttackStyle() == 1) {
						int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(Skills.DEFENCE, defenceXp / 7.5);
						}
					}if (player.getCombatDefinitions().isDefensiveCasting() || (haslightning(player) && player.getCombatDefinitions().getAttackStyle() == 1) && player.getCombatDefinitions().getAttackStyle() == 1) {
						int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(Skills.DEFENCE, defenceXp / 7.5);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (Hit hit : hits) {
					...
					doDefenceEmote();
					int damage = hit.getDamage() > target.getHitpoints() ? target.getHitpoints() : hit.getDamage();
					if (crit && (damage >= max_hit * 0.90) && (hit.getLook() == HitLook.MAGIC_DAMAGE || hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE))
						hit.setCriticalMark();
					if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
						double combatXp = damage / 2.5;
						if (combatXp > 0) {
		...
		for (Hit hit : hits) {
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				...
				if (combatXp > 0) {
					...
					}if (player.getCombatDefinitions().isDefensiveCasting() || (haslightning(player) && player.getCombatDefinitions().getAttackStyle() == 1)) {
						int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(Skills.DEFENCE, defenceXp / 7.5);
					...
					if (hpXp > 0)
						player.getSkills().addXp(Skills.HITPOINTS, hpXp);
				}if (hasFullGuthansEquipped(player)) {
	...
	private boolean hasFullGuthansEquipped(Player player) {
		int helmId = player.getEquipment().getHatId();
		int chestId = player.getEquipment().getChestId();
		int legsId = player.getEquipment().getLegsId();
		int weaponId = player.getEquipment().getWeaponId();
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("bow") || weaponName.contains("decimation"))
					return 426;
				if (weaponName.contains("desert eagle"))
					return 4230;
				if (weaponName.contains("chinchompa"))
					return 2779;
				...
				if (weaponName.contains("staff") || weaponName.contains("wand") || weaponName.contains("oracles_wingblade")  )
					return 419;
				if (weaponName.contains("dart"))
					return 6600;
				if (weaponName.contains("knife"))
					return 9055;
				if (weaponName.contains("scimitar") || weaponName.contains("Jericho's sword")) {
					switch (attackStyle) {
					case 2:
						return 15072;
					default:
				...
				if (weaponName.contains(" sword") || weaponName.contains("saber") || weaponName.contains("longsword") || weaponName.contains("light") || weaponName.contains("excalibur") || weaponName.contains("frostmourne")|| weaponName.contains("corrupt embrace")
|| weaponName.contains("dragongrave scythe")
|| weaponName.contains("supreme escalibur")
|| weaponName.contains("clarent")
|| weaponName.contains("keyblade")
|| weaponName.contains("blades of bond")
|| weaponName.contains("deathbringer")
|| weaponName.contains("soulstealer")
|| weaponName.contains("veren kutsu")
|| weaponName.contains("soulrender scythe")
|| weaponName.contains("ragnarok")
|| weaponName.contains("deus vacuib")
|| weaponName.contains("links sword")
|| weaponName.contains("soul edge")
|| weaponName.contains("fluresence")
|| weaponName.contains("sword of edicts")
|| weaponName.contains("gravity scythe")
|| weaponName.contains("sword of fear")
|| weaponName.contains("ticking death")
|| weaponName.contains("scythe 1")
|| weaponName.contains("scythe 2")
|| weaponName.contains("ecto caliber")
|| weaponName.contains("winged blade")) {
					switch (attackStyle) {
					case 2:
						return 12310;
					default:
				...
				if (weaponName.contains("rapier") || weaponName.contains("brackish") || weaponName.contains("dildo")) {
					switch (attackStyle) {
					case 2:
						return 13048;
					default:
				...
				if (weaponName.contains("triple-bladed scythe")) {
					switch (attackStyle) {
					case 2:
						return 2876;
					default:
				...
				if (weaponName.contains("godsword") || weaponName.contains("flamberge") ) {
					switch (attackStyle) {
					case 2:
						return 11980;
					case 3:
				...
				if (ItemConstants.isDungItem(weaponId) && weaponName.contains("maul")) {
					return 2661;
				}
	...
	private int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			// Interval 2.4
			if (weaponName.equals("zamorakian spear") || weaponName.equals("Jericho's sword"))
				return 2;
			// Interval 3.6
	...
	private boolean checkAll(Player player) {
		...
		if (distanceX < size && distanceX > -1 && distanceY < size && distanceY > -1 && !target.hasWalkSteps()) {
			...
			return true;
		}else if (isRanging == 0 && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && !haslightning(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
			if (!player.addWalkSteps(target.getX(), player.getY(), 1))
				player.addWalkSteps(player.getX(), target.getY(), 1);
			return true;
		}
		maxDistance = isRanging != 0 || player.getCombatDefinitions().getSpellId() > 0 || hasPolyporeStaff(player) || haslightning(player)  ? 7 : 0;
	...
	public static final int isRanging(Player player) {
		...
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart") || weaponId == 6522 || name.contains("javelin") || name.contains("thrownaxe") || name.contains("throwing axe") || name.contains("throwing axe") || name.contains("grave creeper longbow") || name.contains("grave creeper") || name.contains("Grave creeper longbow") || name.contains("Boogie") || name.contains("ecimation") || name.equalsIgnoreCase("zaryte bow") || name.equalsIgnoreCase("desert eagle") || name.equalsIgnoreCase("infernal bow") || name.contains("chinchompa") || name.contains("Bolas"))
				return 2;
		}
		int ammoId = player.getEquipment().getAmmoId();
		switch (weaponId) {
		...
		case 29092:
			...
			case 15775: //Tangle gum shortbow (b)
			case 15776: //Seeping elm shortbow (b)
			case 15777: //Blood spindle shortbow (b)
			case 15778: //Utuku shortbow (b)
			case 15779: //Spinebeam shortbow (b)
			case 15780: //Bovistrangler shortbow (b)
			case 15781: //Thigat shortbow (b)
			case 15782: //Corpsethorn shortbow (b)
			case 15783: //Entgallow shortbow (b)
			case 15784: //Grave creeper shortbow (b)
			case 15785: //Sagittarian shortbow (b)
			case 15903: //Tangle gum longbow (b)
			case 15904: //Seeping elm longbow (b)
			case 15905: //Blood spindle longbow (b)
			case 15906: //Utuku longbow (b)
			case 15907: //Spinebeam longbow (b)
			case 15908: //Bovistrangler longbow (b)
			case 15909: //Thigat longbow (b)
			case 15910: //Corpsethorn longbow (b)
			case 15911: //Entgallow longbow (b)
			case 15912: //Grave creeper longbow (b)
			case 15913: //Sagittarian longbow (b)
			case 16317: //Tangle gum longbow
			case 16319: //Seeping elm longbow
			case 16321: //Blood spindle longbow
			case 16323: //Utuku longbow
			case 16325: //Spinebeam longbow
			case 16327: //Bovistrangler longbow
			case 16329: //Thigat longbow
			case 16331: //Corpsethorn longbow
			case 16333: //Entgallow longbow
			case 16335: //Grave creeper longbow
			case 16337: //Sagittarian longbow
			case 16867: //Tangle gum shortbow
			case 16869: //Seeping elm shortbow
			case 16871: //Blood spindle shortbow
			case 16873: //Utuku shortbow
			case 16875: //Spinebeam shortbow
			case 16877: //Bovistrangler shortbow
			case 16879: //Thigat shortbow
			case 16881: //Corpsethorn shortbow
			case 16883: //Entgallow shortbow
			case 16885: //Grave creeper shortbow
			...
			case 892: // rune arrow
			case 11212: // dragon arrow
			case 16427: //Novite arrows
			case 16432: //Bathus arrows
			case 16437: //Marmaros arrows
			case 16442: //Kratonite arrows
			case 16447: //Fractite arrows
			case 16452: //Zephyrium arrows
			case 16457: //Argonite arrows
			case 16462: //Katagon arrows
			case 16467: //Gorgonite arrows
			case 16472: //Promethium arrows
			case 16477: //Sagittarian arrows
			case 16482: //Novite arrows (p)
			case 16487: //Bathus arrows (p)
			case 16492: //Marmaros arrows (p)
			case 16497: //Kratonite arrows (p)
			case 16502: //Fractite arrows (p)
			case 16507: //Zephyrium arrows (p)
			case 16512: //Argonite arrows (p)
			case 16517: //Katagon arrows (p)
			case 16522: //Gorgonite arrows (p)
			case 16527: //Promethium arrows (p)
			case 16532: //Sagittarian arrows (p)
			case 16537: //Novite arrows (p+)
			case 16542: //Bathus arrows (p+)
			case 16547: //Marmaros arrows (p+)
			case 16552: //Kratonite arrows (p+)
			case 16557: //Fractite arrows (p+)
			case 16562: //Zephyrium arrows (p+)
			case 16567: //Argonite arrows (p+)
			case 16572: //Katagon arrows (p+)
			case 16577: //Gorgonite arrows (p+)
			case 16582: //Promethium arrows (p+)
			case 16587: //Sagittarian arrows (p+)
			case 16592: //Novite arrows (p++)
			case 16597: //Bathus arrows (p++)
			case 16602: //Marmaros arrows (p++)
			case 16607: //Kratonite arrows (p++)
			case 16612: //Fractite arrows (p++)
			case 16617: //Zephyrium arrows (p++)
			case 16622: //Argonite arrows (p++)
			case 16627: //Katagon arrows (p++)
			case 16632: //Gorgonite arrows (p++)
			case 16637: //Promethium arrows (p++)
			case 16642: //Sagittarian arrows (p++)
			case 15947: //Novite arrows (b)
			case 15948: //Bathus arrows (b)
			case 15949: //Marmaros arrows (b)
			case 15950: //Kratonite arrows (b)
			case 15951: //Fractite arrows (b)
			case 15952: //Zephyrium arrows (b)
			case 15953: //Argonite arrows (b)
			case 15954: //Katagon arrows (b)
			case 15955: //Gorgonite arrows (b)
			case 15956: //Promethium arrows (b)
			case 15957: //Sagittarian arrows (b)
			case 15958: //Novite arrows (p) (b)
			case 15959: //Bathus arrows (p) (b)
			case 15960: //Marmaros arrows (p) (b)
			case 15961: //Kratonite arrows (p) (b)
			case 15962: //Fractite arrows (p) (b)
			case 15963: //Zephyrium arrows (p) (b)
			case 15964: //Argonite arrows (p) (b)
			case 15965: //Katagon arrows (p) (b)
			case 15966: //Gorgonite arrows (p) (b)
			case 15967: //Promethium arrows (p) (b)
			case 15968: //Sagittarian arrows (p) (b)
			case 15969: //Novite arrows (p+) (b)
			case 15970: //Bathus arrows (p+) (b)
			case 15971: //Marmaros arrows (p+) (b)
			case 15972: //Kratonite arrows (p+) (b)
			case 15973: //Fractite arrows (p+) (b)
			case 15974: //Zephyrium arrows (p+) (b)
			case 15975: //Argonite arrows (p+) (b)
			case 15976: //Katagon arrows (p+) (b)
			case 15977: //Gorgonite arrows (p+) (b)
			case 15978: //Promethium arrows (p+) (b)
			case 15979: //Sagittarian arrows (p+) (b)
			case 15980: //Novite arrows (p++) (b)
			case 15981: //Bathus arrows (p++) (b)
			case 15982: //Marmaros arrows (p++) (b)
			case 15983: //Kratonite arrows (p++) (b)
			case 15984: //Fractite arrows (p++) (b)
			case 15985: //Zephyrium arrows (p++) (b)
			case 15986: //Argonite arrows (p++) (b)
			case 15987: //Katagon arrows (p++) (b)
			case 15988: //Gorgonite arrows (p++) (b)
			case 15989: //Promethium arrows (p++) (b)
			case 15990: //Sagittarian arrows (p++) (b)
				return 2;
			default:
				return 1;
			}
	...
	private static boolean hasPolyporeStaff(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		return weaponId == 22494 || weaponId == 22496 || weaponId == 28818;
	}
		private static boolean haslightning(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		return weaponId == 28910 || weaponId == 28911 || weaponId == 28912 || weaponId == 28913;
	}
