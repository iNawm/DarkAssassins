...
public class Summoning {
	...
	public static enum Pouches {
		...
		ASSASSIN(8422, -1, 8422, 1, -1, -1, 300000, -1, -1),
		
		CORP(273, 163, 29152, 1, 4.8, 500, 3840000, 10, 12825);
	...
	public static void createPouch(final Player player, int itemId, int amount) {
		...
		if (player.getSkills().getLevelForXp(Skills.SUMMONING) < pouch.getLevelRequired()) {
			player.getPackets().sendGameMessage("You don't have the level required to create that pouch.");
			return;
		}
	...
	public static void infuseScrolls(Player player) {
		...
		Object[] options = new Object[] { 78, 1, "Transform-X<col=ff9040>", "Transform-All<col=ff9040>", "Transform-10<col=ff9040>", "Transform-5<col=ff9040>", "Transform<col=ff9040>", 10, 8, 666 << 16 | 16 };
		player.getPackets().sendRunScript(763, options);
		player.getPackets().sendIComponentSettings(666, 16, 0, 462, 126);
	}

	public static int amtScrollsCanMake(final Player player, int itemId, int amount) {
		SummoningScroll pouch = SummoningScroll.get(itemId);
		return player.getInventory().getItems().getNumberOf(pouch.getPouch());
	}
	...
	public static void transformScrolls(final Player player, int itemId, int amount) {
		...
		int amountToCreate = amtScrollsCanMake(player, itemId, amount);
		if (amount > amountToCreate) {
			amount = player.getInventory().getItems().getNumberOf(itemId);
		}
		if (amount > 28) {
		...
		int i = 0;
		for (i = 0; i < amount; i++) {
			if (!player.getInventory().getItems().contains(scroll.getPouch())) {
				if (amount == 1) {
					player.getPackets().sendGameMessage("You do not have the items required to create this pouch.");
				}
				end = true;
			...
			player.getInventory().removeItems(scroll.getPouch());
			player.getInventory().addItem(scroll.getItemId(), 10);
	...
	public static void sendInterface(Player player) {
		...
		Object options[] = { Integer.valueOf(78), Integer.valueOf(1), "Infuse-All<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-5<col=FF9040>", "Infuse<col=FF9040>", Integer.valueOf(10), Integer.valueOf(8), Integer.valueOf(INTERFACE << 16 | 0x10) };
		player.getPackets().sendRunScript(757, options);
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("com.rs.game.npc.familiar." + (NPCDefinitions.getNPCDefinitions(pouch.getNpcId())).name.replace(" ", "").replace("-", "")).getConstructor(Player.class, Pouches.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
