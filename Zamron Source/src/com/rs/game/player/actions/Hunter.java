...
public class Hunter {
	...
	public enum FlyingEntities {
		...
		BABY_IMPLING(1, 1, 20, 25, 17, new Item[] { new Item(946, 1), new Item(1755, 1), new Item(1734, 1), new Item(1733, 1), new Item(2347, 1), new Item(1985, 1) }, null, null, null),

		YOUNG_IMPLING(1, 1, 48, 65, 22, new Item[] { new Item(361, 1), new Item(1901, 1), new Item(1539, 5), new Item(1784, 4), new Item(1523, 1), new Item(7936, 1), new Item(5970, 1) }, new Item[] {
				new Item(855, 1), new Item(1353, 1), new Item(2293, 1), new Item(7178, 1), new Item(247, 1), new Item(453, 1), new Item(1777, 1), new Item(231, 1), new Item(2347, 1) }, new Item[] {
				new Item(1097, 1), new Item(1157, 1), new Item(8778, 1), new Item(133, 1), new Item(2359, 1), }, null),
		...
		KINGLY_IMPLING(1, 1, 434, 617, 91, new Item[] { new Item(1705, Utils.random(3, 11)), new Item(1684, 3), new Item(1618, Utils.random(17, 34)), new Item(990, 2) }, new Item[] {
				new Item(1631, 1), new Item(1615, 1), new Item(9341, 40 + Utils.getRandom(30)), new Item(9342, 57), new Item(15511, 1), new Item(15509, 1), new Item(15505, 1), new Item(15507, 1),
				new Item(15503, 1), new Item(11212, 40 + Utils.random(104)), new Item(9193, 62 + Utils.random(8)), new Item(11230, Utils.random(182, 319)), new Item(11232, 70),
				new Item(1306, Utils.random(1, 2)), new Item(1249, 1) }, null, new Item[] { new Item(7158, 1), new Item(2366, 1), new Item(6571, 1) }),
		...
		GameEngine.get().addTask(new Task() {
			...
			public void execute() {
				if (isSuccessful(player, instance.getLevel(), new DynamicFormula() {
					...
					@Override
					public int getExtraProbablity(Player player) {
						if (player.getInventory().containsItem(3000, 1) || player.equipment().getItem(3).getId() == 3000)
							return 3;// magic net
						else if (player.getInventory().containsItem(3001, 1) || player.equipment().getItem(3).getId() == 3000)
							return 2;// regular net
						return 1;// hands
					}
				})) {
					player.getSkills().addXp(Skills.HUNTER, player.controlerManager().getControler() instanceof PuroPuro ? instance.getPuroExperience() : instance.getRsExperience());
					npc.finish();
					player.getInventory().addItem(new Item(instance.getReward(), 1));
					player.chat().sendMessage("...and you successfully caputure the " + name.toLowerCase());
				} else {
					if (isImpling) {
						npc.nextForceTalk(new ForceTalk("Tehee, you missed me!"));
						GameEngine.get().addTask(new Task() {
							@Override
							public void execute() {
								Location teleTile = npc.location();
								for (int trycount = 0; trycount < 10; trycount++) {
									teleTile = new Location(npc.location(), 10);
									if (World.canMoveNPC(npc.location().height(), teleTile.x(), teleTile.y(), player.size()))
										break;
									teleTile = npc.location();
								}
								npc.nextLocation(teleTile);
							}
						});
					}
					player.chat().sendMessage("...you stumble and miss the " + name.toLowerCase());
				}
			}
		});
	}
	...
	public static void openJar(Player player, FlyingEntities instance) {
		...
		if (Utils.random(100) > 93) {
			...
			player.chat().sendMessage("You press too hard on the jar and the glass shatters in your hands.");
		}
	}
	...
	public static void createLoggedObject(Player player, GameObject object, boolean kebbits) {
		if (!player.getInventory().containsOneItem(requiredLogs)) {
			player.chat().sendMessage("You need to have logs to create this trap.");
			return;
		}
		player.lock(3);
		player.getActionManager().delay(3);
		...
		if (World.removeTemporaryObject(object, 300000, false)) {// five minute
																	// delay
			World.spawnTemporaryObject(new GameObject(kebbits ? 19206 : -1, object.type(), object.rotation(), object.location()), 300000, false);// TODO
			Item item = null;
