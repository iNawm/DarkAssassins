...
public class Cooking extends Action {
	...
	public enum Cookables {
		 ...
		 * Cooked Meat
		 */
		RAW_MEAT(new Item(2132, 1), 1, 200, 30, new Item(2146, 1), new Item(9436, 1), false, false),
	...
	public int processWithDelay(Player player) {
		...
		if ((player.getSkills().getLevel(Skills.COOKING) >= cook
			...
			player.cookedFish++;
			if (this.cook == Cookables.RAW_LOBSTER){
				player.lobbyscooked++;
			}else if (this.cook == Cookables.RAW_SWORDFISH){
				player.swordfishcooked++;
			}else if (this.cook == Cookables.RAW_MONKFISH){
				player.monkscooked++;
			}else if (this.cook == Cookables.RAW_SHARK){
				player.sharkscooked++;
			}else if (this.cook == Cookables.RAW_MANTA_RAY){
				player.mantascooked++;
			}else if (this.cook == Cookables.RAW_ROCKTAIL){
				player.rocktailscooked++;
			}
