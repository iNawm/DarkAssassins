        ...
        if (stage == -1) {
            ...
            if (componentId == OPTION_1) {
                if (player.getSkills().getCombatLevel() >= 110 && player.getSkills().getLevel(18) >= 75) {
                    ...
                    sendEntityDialogue((short) 241, new String[]{NPCDefinitions.getNPCDefinitions(npcId).name, "Sorry, you need at least level 110 combat and 75 Slayer to use me."}, (byte) 1, npcId, 9827);
                } else if (player.getSkills().getCombatLevel() >= 110 && player.getSkills().getLevel(18) < 75) {
                    sendEntityDialogue((short) 241, new String[]{NPCDefinitions.getNPCDefinitions(npcId).name, "Sorry, you need at least 75 Slayer to use me."}, (byte) 1, npcId, 9827);
                } else if (player.getSkills().getCombatLevel() < 110 && player.getSkills().getLevel(18) >= 75) {
                    sendEntityDialogue((short) 241, new String[]{NPCDefinitions.getNPCDefinitions(npcId).name, "Sorry, you need at least 110 combat to use me."}, (byte) 1, npcId, 9827);
                }
