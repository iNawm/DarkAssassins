...
public class House implements Serializable {
	...
	private final WorldTile location = new WorldTile(2354, 3687, 0);
	...
	public void checkRoom(Player player) {
		...
		RoomReference room = getRoom(X, Y, player.getPlane());// gets room
																// player is in
																// according to
																// coord in
																// house
		...
		CoresManager.slowExecutor.schedule(new Runnable() {
			...
			public void run() {
				MapBuilder.destroyMap(boundChunksCopy[0], boundChunksCopy[1],
						8, 8);
				for (WorldObject object : World.getRegion(MapBuilder.getRegionId(boundChunksCopy[0], boundChunksCopy[1])).getSpawnedObjects())
					if(object != null)
					 World.removeObject(object);
			}
	...
	public void leaveHouse(Player player, int type) {
		...
		if (type == LOGGED_OUT)
			player.setLocation(new WorldTile(Settings.HOME_PLAYER_LOCATION1));
		else if (type == KICKED)
			player.setNextWorldTile(new WorldTile(Settings.HOME_PLAYER_LOCATION1));
