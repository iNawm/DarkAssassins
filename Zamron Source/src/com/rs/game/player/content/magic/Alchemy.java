...
public class Alchemy {
	...
	public static void handleAlchemy(Player player, Item item, boolean lowAlch) {
		...
		if (lowAlch) {
			...
			player.getInventory().addItemMoneyPouch(995, (int) (ItemManager.getPrice(item.getId())/2));
			...
			player.getInventory().deleteItem(item.getId(), 1);
			player.getInventory().addItemMoneyPouch(995, ItemManager.getPrice(item.getId())-(ItemManager.getPrice(item.getId())/3));
