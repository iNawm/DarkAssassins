...
public final class PetManager implements Serializable {
	...
	private boolean hasRequirements(Pets pet) {
		switch (pet) {
		case TZREK_JAD:
			if (!player.isExtremeDonator() || !player.isEliteDonator()) {
		...
		case VULTURE_5:
		case CHAMELEON:
			if (!player.isDonator() || !player.isExtremeDonator() || !player.isEliteDonator()) {
		...
		case WARMING_FLAME:
			if (!player.isExtremeDonator() || !player.isEliteDonator()) {
				player.getPackets().sendGameMessage("You need to be Elite to use this pet.");
				return false;
			}
			return true;
		case FERRET:
