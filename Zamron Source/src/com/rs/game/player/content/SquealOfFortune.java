...
public class SquealOfFortune implements Serializable {
	...
	if (item.getDefinitions().isStackable() || item.getDefinitions().isNoted() || item.getAmount() > 1) {
	    World.sendWorldMessage("<img=6><col=ff0000>News: " + player.getDisplayName() + " has just won " + item.getAmount() + " x " + item.getName() + " on Squeal of Fortune!", false);
	}
	else {
	    World.sendWorldMessage("<img=6><col=ff0000>News: " + player.getDisplayName() + " has just won " + item.getName() + " on Squeal of Fortune!", false);
	}
	
	String moof1 = "has won"; /*
	try {
		AdventureLog.createConnection();
		AdventureLog.query("INSERT INTO `adventure` (`username`,`time`,`action`,`monster`) VALUES ('"+player.getUsername()+"','"+AdventureLog.Timer()+"','"+moof1+"','"+item.getName()+"');");
