...
public class Tortoise extends CombatScript {

	@Override
	public Object[] getKeys() {
	
		return new Object[] {3808};
	}
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if(attackStyle == 1) { //range
			npc.setNextAnimation(new Animation(3954));
			...
			npc.setNextAnimation(new Animation( 3956));
			//npc.setNextGraphics(new Graphics(2995));
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					World.sendProjectile(npc, target, 102, 80, 30, 40, 20, 5, 0);
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
