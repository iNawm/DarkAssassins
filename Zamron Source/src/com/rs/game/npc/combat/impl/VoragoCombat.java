	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.getRandom(3) == 0) {
			for (Entity t : npc.getPossibleTargets()) {
				...
				delayHit(npc, 0, t, getRegularHit(npc, getRandomMaxHit(npc, 500, NPCCombatDefinitions.SPECIAL, t)));
				World.sendProjectile(npc, t, 3096, 41, 16, 41, 35, 16, 0);
			}
		}
		if (Utils.getRandom(10) == 0) {//Huge Ranged Attack
			for (Entity t : npc.getPossibleTargets()) {
					npc.setNextAnimation(new Animation(12205));
			...
			for (Entity t : npc.getPossibleTargets()) {
				npc.setNextAnimation(new Animation(12210));
