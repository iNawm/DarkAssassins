...
public class GraveStone extends NPC {// 652 - gravestone selection interface
	...
	graveStone1 = player.getGraveStone();
	setDirection(Utils.getFaceDirection(0, -1));
	setNextAnimation(new Animation(graveStone1 == 1 ? 7396 : 7394));
	username = player.getUsername();
	ticks = getMaximumTicks(graveStone1);
    ...
    public static int getMaximumTicks(int graveStone1) {
	switch (graveStone1) {
	    case 0:
		return 500;
	    case 1:
	...
	Collections.sort(droppedItems, new Comparator<Integer>() {
		...
		int price1 = ItemManager.getPrice(i1.getId());
		int price2 = ItemManager.getPrice(i2.getId());
		if (price1 > price2)
		    return -1;
		if (price1 < price2)
		    return 1;
